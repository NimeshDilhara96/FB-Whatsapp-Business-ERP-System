...
export default function Sidebar({ isOpen, onClose }) {
    ...
    const navItemClass = "flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-tx-muted hover:text-primary-700 hover:bg-primary-50 transition-colors cursor-pointer";
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <div className={`fixed inset-y-0 left-0 w-64 bg-base-surface border-r border-base-border flex flex-col shadow-sm z-50 transform transition-transform duration-300 md:relative md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                <div className="h-16 flex items-center justify-between px-6 border-b border-base-border-subtle shrink-0">
                    <div className="flex items-center gap-2.5">
                        ...
                        <h2 className="text-xl font-bold text-tx-main tracking-tight">SocialERP</h2>
                    </div>
                    {/* Close button (Mobile only) */}
                    <button onClick={onClose} className="md:hidden text-tx-subtle hover:text-tx-main">
